	...
	class Road
	{
		...
		public void Reset()
		{
			...
			if (CullBack)
			{
				for (int i = 0; i < 10; i++)
				{
					var piece = new RoadSegment(content, world, Size * (i - 1), groundWidth, groundOffsetX, biomeScale);
		...
		public void Update(float playerY)
		{
			if (CullBack && playerY - Segments.First.Value.Y > Size*2)
			{
				...
				var piece = new RoadSegment(content, world, Segments.Last.Value.Y + Size, groundWidth, groundOffsetX, biomeScale);
				Segments.AddLast(piece);
			} else if (!CullBack && Segments.First.Value.Y < playerY)
			{
				Segments.Last.Value.Destroy();
				Segments.RemoveLast();
				var piece = new RoadSegment(content, world, Segments.First.Value.Y + Size, groundWidth, groundOffsetX, biomeScale);
